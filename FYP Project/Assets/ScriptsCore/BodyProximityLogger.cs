...
public class BodyProximityLogger : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!_isNear && dist <= enterRadius && !deadBodyCounted)
        {
            _isNear = true;
            Debug.Log($"[Proximity] ENTER near '{name}'. Dist = {dist:F2} m");
            Marker.SetActive(true);
            InputsController.Instance.evidenceCollected++;
            Marker.GetComponent<MaarkerEntity>().SetMarkerNumber(InputsController.Instance.evidenceCollected);
            EvidenceListController.instance.SetEvidenceCollected(EvidenceListController.DeadBody);
            deadBodyCounted = true;
